...
import { Trophy, Users, RefreshCcw, ListPlus } from 'lucide-react';

export const SorteadorNomes: React.FC = () => {
    ...
    <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-12 gap-8">
      ...
      <section className="lg:col-span-4 space-y-6">
        <div className="bg-slate-800/50 p-6 rounded-3xl border border-slate-700 shadow-xl">
          ...
          <textarea 
            className="w-full h-48 bg-slate-900/50 border border-slate-700 rounded-xl p-4 focus:ring-2 focus:ring-blue-500 outline-none transition-all resize-none text-sm text-slate-200"
          ...
          <button 
            ...
            className="w-full mt-4 bg-blue-600 hover:bg-blue-500 py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-all active:scale-95 text-white cursor-pointer"
          >
        ...
        <div className="bg-slate-800/50 p-6 rounded-3xl border border-slate-700">
          ...
          <input 
            type="number" 
            min="1" 
            className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-xl font-bold text-center text-white"
            value={quantidade}
      ...
      <section className="lg:col-span-8 flex flex-col gap-6">
        ...
        <div className="relative bg-gradient-to-br from-blue-700 to-indigo-900 rounded-[2.5rem] p-12 shadow-2xl min-h-[400px] flex flex-col items-center justify-center overflow-hidden">
          ...
          {estaSorteando ? (
            ...
            <div className="text-center opacity-40 text-slate-200">
        ...
        <button 
          ...
          onClick={sortear}
          className="w-full bg-emerald-500 hover:bg-emerald-400 disabled:bg-slate-800 disabled:text-slate-600 py-8 rounded-[2rem] text-4xl font-black shadow-xl transition-all active:scale-[0.98] uppercase tracking-tighter text-white cursor-pointer"
        >
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-slate-800 p-5 rounded-3xl border border-slate-700">
            <span className="text-xs text-slate-400 uppercase font-bold tracking-widest">Disponíveis</span>
            <p className="text-3xl font-black text-white">{disponiveis.length} <span className="text-sm font-normal text-slate-400">/ {todosOsNomes.length}</span></p>
          </div>
          <button 
            onClick={reiniciar}
            className="bg-slate-800 hover:bg-slate-700 p-5 rounded-3xl border border-slate-700 flex items-center justify-center gap-3 font-bold transition-all active:scale-95 text-white cursor-pointer"
          >
        ...
        <div className="bg-slate-800/30 p-6 rounded-3xl border border-slate-700 h-64 overflow-hidden flex flex-col">
          <h4 className="text-xs font-black text-slate-400 mb-4 uppercase tracking-[0.2em]">Últimos Ganhadores</h4>
          <div className="overflow-y-auto space-y-2 flex-1 scrollbar-hide">
            ...
            )) : <p className="text-center text-slate-500 mt-10 text-sm">Nenhum sorteio realizado ainda.</p>}
          </div>
        </div>
      </section>
    </div>
